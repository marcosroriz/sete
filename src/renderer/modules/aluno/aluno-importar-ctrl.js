...
$('#importarAlunos').click(() => {
  ...
  readXlsxFile(xlsFilePath, { schema }).then(({ rows, errors }) => {
      ...
      rows.forEach(linha => {        
        ExisteAluno(linha["NOME"], linha["NOME_RESPONSAVEL"], linha["grauResponsavel"])
          .then((res, rej) => {
            if(res.length > 0){
              throw new Error("Opa!");
            } else {
          ...
          .catch(err => {
            console.log("Linha da planilha existente no banco")
            swal.close();
            Swal2.fire({
              title: "Atenção",
              text: "Alguns alunos da planilha constam no sistema. " +
                  "Clique abaixo para retornar ao painel.",
              type: "warning",
              icon: "warning",
              showCancelButton: false,
