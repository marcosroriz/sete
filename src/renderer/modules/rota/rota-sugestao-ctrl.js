...
var mapaConfig = novoMapaOpenLayers(
  "mapaRotaSugestaoConfig",
  cidadeLatitude,
  cidadeLongitude
);
var mapaRotaGerada = novoMapaOpenLayers(
  "mapaRotaSugestaoGerada",
  cidadeLatitude,
  cidadeLongitude
);
    ...
    loadOSMFile()
      ...
      .catch((err) => {
        ...
        if (code == "erro:malha") {
          ...
          informarNaoExistenciaDado(
            "Não há nenhum aluno georeferenciado",
            "Gerenciar alunos",
...
async function preprocessarGaragem() {
  try {
    ...
    let garagemJSON = await restImpl.dbGETEntidade(
      DB_TABLE_GARAGEM,
      `/${idGaragem}`
    );
...
function getPoint(stopType, stopID) {
  ...
  return new ol.geom.Point(
    ol.proj.fromLonLat([parada["lng"], parada["lat"]])
  ).getCoordinates();
}
...
function drawRoutes(routesJSON) {
  ...
  routesJSON.forEach((r) => {
    ...
    let camada = mapaRotaGerada["createLayer"](
      r["id"],
    ...
    let estTime = Number(
      (r["geojson"]?.properties?.travDistance / $("#velMedia").val()) * 60
    )?.toFixed(2);
    if (estTime) {
    ...
    // Add Route Drawing
    let gjson = new ol.format.GeoJSON({
...
// função para obter dados da frota
// https://sweetalert2.github.io/
async function choiceFC() {
  ...
  if (escolha == 0) {
    const { value: formValues } = await Swal.fire({
      title: "FROTA HOMOGÊNEA",
      html:
        '<div>Quantidade</div><input id="swal-input1" class="swal2-input">' +
        '<div>Capacidade</div><input id="swal-input2" class="swal2-input">',
      focusConfirm: false,
      preConfirm: () => {
        return [
    ...
    new_capacity = Array();
    for (i = 0; i < parseInt(formValues[0]); i++) {
      new_capacity.push(formValues[1]);
    }
    return new_capacity;
  } else if (escolha == 1) {
    let capacitys = Array();
    while (true) {
      ...
      capacitys.push(formValues);

      // CRIA TEXTO PARA INSERIR NAS MENSAGENS
      capacity = Array();
      capacitys.forEach(function e(item) {
        if (parseInt(item[0]) == 1) {
          capacity.push(`${item[0]} FROTA DE ${item[1]} ACENTOS`);
        } else {
          capacity.push(`${item[0]} FROTAS DE ${item[1]} ACENTOS`);
        }
      });

      const { value: formValues2 } = await Swal.fire({
        title: "INCLUIR MAIS UMA FROTA?",
        text: capacity.join(", ") + ".",
        icon: "question",
        showDenyButton: true,
        confirmButtonText: "+ 1",
        denyButtonText: "Iniciar Simulação",
        denyButtonColor: "#23CCEF",
        focusConfirm: false,
        preConfirm: (result) => {
      ...
      if (formValues2 == false) {
        break;
      }
    }
    new_capacity = Array();
    capacitys.forEach(function e(item) {
      for (i = 0; i < parseInt(item[0]); i++) {
        new_capacity.push(item[1]);
      }
    });
    capacitys = new_capacity;
    return capacitys;
  } else if (escolha == 2) {
    return Swal.fire({ html: "You selected: FNDE" });
  }
}
capacitys = await choiceFC();
//capacitys;
...
if (isElectron) {
  ...
  ipcRenderer.on("end:route-generation", function (evt, routesJSON) {
    setTimeout(function () {
      ...
      // Ativa grupo
      let switcher = mapaRotaGerada["activateSidebarLayerSwitcher"](
        ".sidebar-RotasGeradas"
      );
      ...
      switcher.on("drawlist", function (e) {
        $(".ol-layer-vector").each((_, li) => {
          ...
          if (!temBadge) {
            let rotaID = Number(
              $($(li).find("label")[0]).text().split(": ")[1]
            );
            $(
              $.parseHTML(
                '<div class="badge badge-pill badge-warning pull-right"><i class="fa fa-map-o"></i></div>'
              )
            )
              .on("click", function () {
                mapaRotaGerada["map"].getView().fit(gSource.getExtent(), {
...
function replotaDados() {
  ...
  let alunosFiltrados = [...alunoMap].filter((a) => {
    ...
    if (filtroSemRota) {
      alunoFiltrado =
        a[1]["GPS"] &&
        a[1]["ESCOLA_TEM_GPS"] &&
        a[1]["TURNO"] == filtroTurno &&
        a[1]["TEM_ROTA"] == false;
    } else {
      alunoFiltrado =
        a[1]["GPS"] && a[1]["ESCOLA_TEM_GPS"] && a[1]["TURNO"] == filtroTurno;
    }
  ...
  let escolasFiltradas = [...escolaMap].filter(
    (e) => escolasAlunosFiltrados.has(e[1]["ID"]) && e[1]["GPS"]
  );
...
$("#rota-sugestao-saveBtnSim").on("click", () => {
  ...
  if (numRotas == 0) {
    ...
    let rotasLabels = [
      "<strong>Note que os alunos selecionados serão transferidos para as rotas selecionadas.</strong>",
    ];
      ...
      if (res.isConfirmed) {
        ...
        rotasSelecionadas.forEach((rID) => {
          ...
          if (rg) {
            ...
            let rgEscolas = rg.payload.path.filter(
              (k) => k.type == "otherschool" || k.type == "school"
            );
        ...
        function updateProgresso() {
          progresso++;
          let progressoPorcentagem = Math.round(
            100 * (progresso / totalOperacoes)
          );
            ...
            for (r of rotasQueVamosSalvar) {
              r.alunos.forEach((a) =>
                promiseArrayRelacoes.push(
                  dbInserirPromise(DB_TABLE_ROTA_ATENDE_ALUNO, {
                    ID_ROTA: r.id,
                    ID_ALUNO: a.id,
              ...
              r.escolas.forEach((e) =>
                promiseArrayRelacoes.push(
                  dbInserirPromise(DB_TABLE_ROTA_PASSA_POR_ESCOLA, {
                    ID_ROTA: r.id,
                    ID_ESCOLA: e.id,
                  }).then(() => updateProgresso())
              ...
              promiseArrayRelacoes.push(
                dbInserirPromise(DB_TABLE_ROTA, rotaJSON, r.id).then(() =>
                  updateProgresso()
                )
              );
              promiseArrayRelacoes.push(
